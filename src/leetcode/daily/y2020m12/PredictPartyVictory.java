...
public class PredictPartyVictory {
    ...
    public String predictPartyVictory(String senate) {
        ...
        while (true) {
            ...
            for (int i = 0; i < chars.length; i++) {
                if (flag[i]) {
                    if (chars[i] == 'R') {
                        R.addLast(R.pollFirst());
                        flag[D.pollFirst()]=false;
                        if(D.isEmpty()){
                            return "Radiant";
                        }
                    } else {
                        D.addLast(D.pollFirst());
                        flag[R.pollFirst()]=false;
                        if(R.isEmpty()){
                            return "Dire";
                        }
                    }
    ...
    public void test() {
        ...
        Assert.assertEquals("Radiant", predictPartyVictory("RRDDD"));
        Assert.assertEquals("Radiant", predictPartyVictory("RD"));
        Assert.assertEquals("Dire", predictPartyVictory("RDD"));
        Assert.assertEquals("Dire", predictPartyVictory("D"));
        Assert.assertEquals("Radiant", predictPartyVictory("R"));
